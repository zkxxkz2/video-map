...
LOCAL_COSYVOICE_PREFIX = "local-cosyvoice:"
...
def is_local_cosyvoice_voice(voice_name: str):
    """检查是否是本地 CosyVoice 声音"""
    return voice_name.startswith(LOCAL_COSYVOICE_PREFIX)


def get_local_cosyvoice_voices() -> list[str]:
    """
    从本地 CosyVoice 服务拉取可用音色列表
    """
    base_url = config.local_tts.get("base_url", "http://127.0.0.1:9880").rstrip("/")
    try:
        response = requests.get(f"{base_url}/voices", timeout=10)
        response.raise_for_status()
        payload = response.json()
        voices = payload.get("voices", [])
        return [f"{LOCAL_COSYVOICE_PREFIX}{voice}" for voice in voices]
    except Exception as e:
        logger.warning(f"failed to fetch local cosyvoice voices: {str(e)}")
        return []


def get_audio_extension(voice_name: str) -> str:
    if is_local_cosyvoice_voice(voice_name):
        return ".wav"
    return ".mp3"


def _build_sentence_submaker(text: str, audio_duration: float) -> SubMaker:
    sub_maker = SubMaker()
...
def local_cosyvoice_tts(
    ...
    """
    调用本地 CosyVoice 服务进行 SFT 预置音色合成。
    当前只依赖内置说话人，不做克隆和训练。
    """
    del voice_volume
    base_url = config.local_tts.get("base_url", "http://127.0.0.1:9880").rstrip("/")
    speaker = voice_name.replace(LOCAL_COSYVOICE_PREFIX, "", 1)
    payload = {
        "text": text.strip(),
        "speaker": speaker,
        "speed": voice_rate,
    ...
    try:
        logger.info(f"start local cosyvoice tts, speaker: {speaker}")
        response = requests.post(
            f"{base_url}/synthesize", json=payload, timeout=(30, 600)
        )
        response.raise_for_status()
        ...
        if audio_duration <= 0:
            logger.error("local cosyvoice tts succeeded but audio duration is invalid")
            return None

        logger.success(f"local cosyvoice tts succeeded: {voice_file}")
        return _build_sentence_submaker(text, audio_duration)
    except Exception as e:
        logger.error(f"local cosyvoice tts failed: {str(e)}")
        return None
    ...
    for i in range(3):  # 尝试3次
        try:
            ...
            if response.status_code == 200:
                ...
                audio_duration = get_audio_duration(voice_file)
                if audio_duration <= 0:
                    logger.error("siliconflow tts succeeded but audio duration is invalid")
                    return None

                sub_maker = _build_sentence_submaker(text, audio_duration)
...
def _get_audio_duration_from_mp3(mp3_file: str) -> float:
    ...
    if not os.path.exists(mp3_file):
        logger.error(f"Audio file does not exist: {mp3_file}")
        return 0.0
